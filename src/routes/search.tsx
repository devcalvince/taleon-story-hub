...
function SearchPage() {
  ...
  const nothing =
    !results.stories.length && !results.chapters.length && !results.characters.length && !results.genres.length;
    ...
    <>
      ...
      <div className="mx-auto w-full max-w-7xl px-4 sm:px-6">
        ...
        >
          ...
          />
          <button className="rounded-md bg-gold px-6 py-3 text-sm font-medium text-gold-foreground">Search</button>
        </form>
      </div>
      ...
      <div className="mx-auto w-full max-w-7xl space-y-12 px-4 py-12 sm:px-6">
        {!q ? (
          ...
          <EmptyState title="No results" body={`Nothing matched “${q}”. Try a different title, character or genre.`}>
                  ...
                  {(results.chapters as any[]).map((c) => (
                    <li key={c.id}>
                      <Link
                        ...
                        params={{ slug: c.stories?.slug ?? "", chapterNumber: String(c.chapter_number) }}
