...
function AdminLocationsPage() {
  ...
  useEffect(() => { if (isAdmin) { fetchLocations(); fetchStories(); } }, [isAdmin]);

  async function fetchLocations() {
    setLoadingData(true);
    const { data } = await supabase.from("locations").select("*, stories(title, slug)").order("name");
    setLocations(data ?? []); setLoadingData(false);
  }
  ...
  async function handleSave() {
    ...
    setSaving(true);
    const data = { story_id: fStory, name: fName, description: fDesc || null, visual_prompt: fVisualPrompt || null, reference_image_url: fRefImage || null, notes: fNotes || null };
    if (editing) {
    ...
    setDialogOpen(false); setSaving(false); fetchLocations();
  }
              ...
              <div key={loc.id} className="rounded-lg border border-border bg-surface-2 p-4">
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex gap-1 flex-shrink-0">
                    ...
                    <button onClick={() => deleteLocation(loc.id)} className="p-1 text-muted-foreground hover:text-red-500"><Trash2 className="h-4 w-4" /></button>
                  </div>
                </div>
              </div>
            ))}
